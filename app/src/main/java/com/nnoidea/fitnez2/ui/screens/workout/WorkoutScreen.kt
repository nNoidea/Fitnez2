...
fun WorkoutScreen(
    ...
    val exercises by exerciseService.getAllExercisesFlow().collectAsState(initial = null)
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Column(
            ...
            val historyItems = remember(workoutItems.toList()) {
                prepareRecordDisplayItems(workoutItems)
            }
            RecordList(
                items = historyItems,
                listState = listState,
                ...
                onDeleteRequest = { record ->
                    workoutItems.removeAll { it.workoutRecord.id == record.id }
                },
                onUpdateRequest = { record ->
                    val index = workoutItems.indexOfFirst { it.workoutRecord.id == record.id }
                    if (index != -1) {
                        workoutItems[index] = workoutItems[index].copy(
                            workoutRecord = workoutItems[index].workoutRecord.copy(
                                sets = record.sets,
                                reps = record.reps,
